...
MODEL_PRESETS = {
    "all": list(VALID_MODEL_NAMES),
    "proposed_only": ["proposed"],
    "baselines_only": ["uniform_weight", "reconstruction_only", "regularization_only", "label_bce"],
}


def _resolve_requested_models(raw_value: str) -> list[str]:
    ...
    invalid = [name for name in tokens if name not in VALID_MODEL_NAMES]
    if invalid:
        raise ValueError(
            f"Unsupported model name(s): {invalid}. "
            f"Valid names: {list(VALID_MODEL_NAMES)}. "
            f"Or use presets: {list(MODEL_PRESETS.keys())}."
        )

    unique_tokens: list[str] = []
    for name in tokens:
        if name not in unique_tokens:
            unique_tokens.append(name)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="all",
        help=(
            "Model selection. Use presets {all, proposed_only, baselines_only} "
            "or a comma-separated subset from "
            "{proposed,uniform_weight,reconstruction_only,regularization_only,label_bce}."
        ),
    )
    parser.add_argument(
        "--input-data",
        ...
        dest="input_data",
        help=(
            "Optional path to a pickle file containing a dict with keys "
            "{'X', 'y', 'traj'}. traj must include traj['s']."
        ),
    )
    parser.add_argument(
        "--skip-sweeps",
        action="store_true",
        help="Skip perturbation sweeps and only run training/evaluation.",
    )
    ...
    try:
        args.models = _resolve_requested_models(args.models)
...
def sweep_direction_parallel_twosample_ks(
    *,
    ...
    latent_specs = {name: latent_specs_all[name] for name in latent_models.keys()}
    ...
    for i, delta in enumerate(deltas):
        ...
        array_keys = []
        for spec in latent_specs.values():
            array_keys.extend([spec["pval_col"], f"ks_p_{spec['prefix']}"])

        if label_model is not None:
            array_keys.extend(["pval_label", "label_prob_base", "label_prob_shift"])

        # Disabled for now: only keep KS two-sample outputs.
        # The commented metrics below are kept for future extension.
        disabled_array_keys = [
            # Disabled for now: only keep KS two-sample outputs.
            # "cvm_p_prop",
            # "cvm_p_base",
...
def _load_external_data(path_value: str) -> tuple[np.ndarray, np.ndarray, dict]:
    data_path = Path(path_value)
    if not data_path.exists():
        raise FileNotFoundError(f"--input-data not found: {data_path}")
    if data_path.suffix.lower() not in {".pkl", ".pickle"}:
        raise ValueError(
            "--input-data currently supports pickle files only (.pkl/.pickle). "
            "Expected a dict with keys {'X', 'y', 'traj'}."
        )
    ...
    if not isinstance(payload, dict):
        raise TypeError("External data payload must be a dict")

    missing = [k for k in ("X", "y", "traj") if k not in payload]
    if missing:
        raise KeyError(f"External data missing keys: {missing}. Required: ['X', 'y', 'traj']")
    ...
    if not isinstance(traj, dict):
        raise TypeError("payload['traj'] must be a dict and include key 's'")
    if "s" not in traj:
        raise KeyError("payload['traj'] must include key 's' with shape (N, T)")
    ...
    if X.ndim != 2:
        raise ValueError(f"X must have shape (N, d_x). Got shape={X.shape}")
    if y.ndim != 1:
        raise ValueError(f"y must have shape (N,). Got shape={y.shape}")
    if s.ndim != 2:
        raise ValueError(f"traj['s'] must have shape (N, T). Got shape={s.shape}")

    n = int(X.shape[0])
    if int(y.shape[0]) != n or int(s.shape[0]) != n:
        raise ValueError(
            "X, y, and traj['s'] must share the same first dimension N. "
            f"Got X={X.shape}, y={y.shape}, s={s.shape}"
        )
    ...
    if not np.all(np.isin(y_unique, [0.0, 1.0])):
        raise ValueError(
            ...
            f"Found unique values: {y_unique.tolist()}"
        )
...
def main() -> None:
    ...
    print(f"  input_data        : {cfg.input_data}")
    print(f"  skip_sweeps       : {cfg.skip_sweeps}")
    ...
    if cfg.input_data is None:
        ...
        _print_stage("Loading external dataset")
        x_np, y, traj = _load_external_data(cfg.input_data)
        s = np.asarray(traj["s"], dtype=np.float32)
        cfg.n_samples = int(x_np.shape[0])
        if int(cfg.d_x) != int(x_np.shape[1]):
            print(f"[Data] Overriding d_x from {cfg.d_x} to {x_np.shape[1]}")
            cfg.d_x = int(x_np.shape[1])

    o_hist, o_next = make_sequence_pairs(s=s, stride=cfg.stride)
    if int(cfg.d_o) != int(o_hist.shape[-1]):
        print(f"[Data] Overriding d_o from {cfg.d_o} to {o_hist.shape[-1]}")
    ...
    seq_end = float(sequence_dt * o_next.shape[1])
    requested_start, requested_end = float(cfg.time_window[0]), float(cfg.time_window[1])
    if requested_start > requested_end:
        raise ValueError(
            f"Invalid time_window={cfg.time_window}: start must be <= end."
        )
    if requested_start < seq_start or requested_end > seq_end:
        raise ValueError(
            "time_window and trajectory range mismatch. "
            f"Requested window={cfg.time_window}, "
            f"but valid sequence time range is [{seq_start:.4f}, {seq_end:.4f}]. "
            "Please pass a valid --time-window."
        )
    ...
    _print_stage(f"Training selected models: {selected_models}")

    history_by_model: dict[str, pd.DataFrame] = {}
    sequence_model_configs = {
        "proposed": proposed_cfg,
        "uniform_weight": uniform_weight_cfg,
        "reconstruction_only": recon_only_cfg,
    }
    trained_sequence_models: dict[str, nn.Module] = {}

    for model_name in ("proposed", "uniform_weight", "reconstruction_only"):
        if model_name not in selected_models:
            continue
        _print_stage(f"Training {model_name}")
        model, history = train_sequence_model(
            ...
            device=device,
            config=sequence_model_configs[model_name],
        )
        trained_sequence_models[model_name] = model
        history_by_model[model_name] = history

    reg_only_model: nn.Module | None = None
    if "regularization_only" in selected_models:
        ...
        history_by_model["regularization_only"] = reg_only_history
        trained_sequence_models["regularization_only"] = reg_only_model

    label_model: nn.Module | None = None
    if "label_bce" in selected_models:
        ...
        history_by_model["label_bce"] = label_history

    latent_models = {name: model for name, model in trained_sequence_models.items()}
    ...
    if cfg.skip_sweeps:
        _print_stage("Skipping perturbation sweeps (--skip-sweeps)")
    elif not latent_models:
        _print_stage("Skipping perturbation sweeps (no latent models selected)")
    elif int(cfg.d_x) != 2:
        _print_stage("Skipping perturbation sweeps (only supported for d_x=2)")
    elif cfg.perturb_type == "mean":
    ...
    if dfs:
        ...
        method_to_pval_col = {}
        if "proposed" in latent_models:
            method_to_pval_col["proposed"] = "pval"
        if "regularization_only" in latent_models:
            method_to_pval_col["regularization_only"] = "pval_base"
        if "reconstruction_only" in latent_models:
            method_to_pval_col["reconstruction_only"] = "pval_recon"
        if "uniform_weight" in latent_models:
            method_to_pval_col["uniform_weight"] = "pval_uniform"
        if label_model is not None:
            method_to_pval_col["label_bce_binom"] = "pval_label"
        ...
        if label_model is not None:
            label_probability_summary = pd.concat(
                [
                    df[
                        [
                            "delta",
                            "label_prob_base_mean",
                            "label_prob_base_std",
                            "label_prob_shift_mean",
                            "label_prob_shift_std",
                        ]
            ...
            label_probability_summary = pd.DataFrame()

        if "proposed" in latent_models and "regularization_only" in latent_models:
            gap_df, gap_table, mean_summary = build_compat_mean_summary(
                df1=dfs["df1"],
                df2=dfs["df2"],
                df3=dfs["df3"],
                alpha=0.1,
            ...
            mean_summary = pd.DataFrame(columns=["metric", "value"])
    else:
        scalar_summary_df = pd.DataFrame()
        rejection_rate_summary = pd.DataFrame()
        label_probability_summary = pd.DataFrame()
        gap_df = pd.DataFrame()
        gap_table = pd.DataFrame()
        mean_summary = pd.DataFrame(columns=["metric", "value"])
    ...
    for name, hist_df in history_by_model.items():
    ...
    for name in ("proposed", "uniform_weight", "reconstruction_only"):
        if name not in trained_sequence_models:
            continue
        model = trained_sequence_models[name]
    ...
    for setting_name, df in dfs.items():
        df.to_pickle(output_dir / f"{setting_name}.pkl")
    ...
    gap_table.to_csv(output_dir / "gap_table.csv", index=True)
