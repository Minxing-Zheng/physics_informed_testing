...
def one_shot_sine(t, X_core):
    A = float(X_core[0])
    omega = abs(float(X_core[1])) + 1e-6
    T = 2.0 * math.pi / omega
    return A * math.sin(omega * t) if 0.0 <= t <= T else 0.0


def one_pulse(t, X_core):
    A = float(X_core[0])                  # 用 x1 当脉冲强度
    pulse_width = 0.05                    # 50ms 脉冲（你也可以设成 dt 或 2*dt）
    return A if 0.0 <= t <= pulse_width else 0.0

# ----------------------------
# Example / Demo (You can delete this section in your repo if unwanted)
# ----------------------------

def _example_config_P0(seed: int = 7) -> GeneratorConfig:
    ...
    C_FIXED = 0.30
    aux_c = NormalSpec(mean=C_FIXED, std=0.0)
    ...
    # forcing = ForcingMap(mode="custom", custom=one_shot_sine)
    forcing = ForcingMap(mode="custom", custom=one_pulse)
...
def _example_config_P1(seed: int = 8) -> GeneratorConfig:
    safe = SafeSetParams(mu=(0.0, 0.0), radius=2.0)
...
def main_demo(output_dir: str = "./toy_outputs") -> None:
    _ensure_dir(output_dir)

    # Generate P0 and P1
    cfg0 = _example_config_P0(seed=7)
    gen0 = SpringScenarioGenerator(cfg0)
    ds0 = gen0.generate()
    ds0.save_npz(os.path.join(output_dir, "dataset_P0.npz"))

    cfg1 = _example_config_P1(seed=8)
