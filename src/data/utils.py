...
from data.stability import check_safety as _check_safety_stability
...
def check_safety(
    ...
    """
    return _check_safety_stability(
        traj=traj,
        threshold=threshold,
        window=window,
        use_abs=True,
...
def _mu_plot_style(mode: str, cmap, vmin, vmax):
    """Choose a clearer default style for binary vs probability safety maps."""
    if mode == "indicator":
        if cmap is None:
            cmap = ListedColormap(["#d97b66", "#4c9f70"])
        norm = BoundaryNorm([-0.5, 0.5, 1.5], cmap.N) if isinstance(cmap, ListedColormap) else None
        return cmap, norm, 0.0 if vmin is None else vmin, 1.0 if vmax is None else vmax, [0, 1]

    if cmap is None:
        cmap = "viridis"
    return cmap, None, vmin, vmax, None
...
#     cmap, norm, vmin, vmax, cbar_ticks = _mu_plot_style(mode, cmap, vmin, vmax)

#     plt.figure()
#     mesh_kwargs = dict(cmap=cmap, shading="auto", vmin=vmin, vmax=vmax, norm=norm)
#     if show_cell_edges:
...
def _eval_one_cov_cell(
    ...
    *,
    ...
    """
    Evaluate p_safe for one covariance cell (j, i). Returns (j, i, p_safe).
    """
    X = sample_x(
        n_samples=n_per_cov,
        dist_type="gaussian",
        params={"mean": mean, "cov": sigma},
        seed=seed,
        truncate_box=truncate_X_box,
    )
    ...
    return j, i, safe_count / float(n_per_cov)


def compute_cov_safety_map(
    params,
    t_final: float,
    dt: float,
    window: Tuple[float, float],
    ...
    mean: np.ndarray,
    n_cov_grid: int = 21,
    delta1_limits: tuple[float, float] = (0.2, 2.0),
    delta2_limits: tuple[float, float] = (-0.9, 0.9),
    n_per_cov: int = 10_000,
    forcing_type: str = "sin",
    config: np.ndarray | None = None,
    eta: float = 0.1,
    truncate_X_box: Optional[tuple[float, float]] = None,
    ...
    """
    Estimate p_safe(delta1, delta2) on a covariance grid where

        Sigma(delta1, delta2) = [[delta1, delta2], [delta2, 1]]

    Here delta1 is the variance of the first feature and delta2 is the
    correlation/covariance term with the second feature variance fixed at 1.
    Valid cells satisfy delta1 > delta2**2.
    """
    mean = np.asarray(mean, float).reshape(2)

    delta1_vals = np.linspace(delta1_limits[0], delta1_limits[1], n_cov_grid)
    delta2_vals = np.linspace(delta2_limits[0], delta2_limits[1], n_cov_grid)

    safe_grid = np.zeros((n_cov_grid, n_cov_grid), dtype=float)
    valid_mask = np.ones((n_cov_grid, n_cov_grid), dtype=bool)

    tasks = []
    seeds = np.random.SeedSequence(seed).spawn(n_cov_grid * n_cov_grid)
    idx = 0

    for j, d1 in enumerate(delta1_vals):
        for i, d2 in enumerate(delta2_vals):
            sigma = np.array([[d1, d2], [d2, 1.0]], dtype=float)

            if d1 <= 0.0 or d1 <= d2**2:
                valid_mask[j, i] = False
                idx += 1
                continue

            tasks.append((j, i, sigma, seeds[idx]))
            idx += 1

    def _run(task):
        j, i, sigma, ss = task
        return _eval_one_cov_cell(
            j=j,
            i=i,
            mean=mean,
            params=params,
    ...
    threshold_value = 1.0 - float(eta)
    config_safe_grid = np.full_like(safe_grid, np.nan)
    config_safe_grid[valid_mask] = (safe_grid[valid_mask] >= threshold_value).astype(float)
